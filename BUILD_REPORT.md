...
| A07 | ID & Auth Failures        | PARTIAL — email+password + Argon2id + lockout + account-existence oracle defense done; short JWT TTL enforced. MFA + email verification deferred per Master Spec §2. Idle timeout, forced re-auth, and refresh-token rotation are **planned, not yet enforced** — `SHIELD_IDLE_TIMEOUT_SECONDS`/`SHIELD_FORCED_REAUTH_SECONDS` are loaded but read by no enforcement path (deferred to the MFA package, DECISIONS D-017) |
