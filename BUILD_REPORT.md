...
| A07 | ID & Auth Failures        | PARTIAL — email+password + Argon2id + lockout + account-existence oracle defense done; short JWT TTL enforced. MFA + email verification deferred per Master Spec §2. Refresh-token rotation with reuse detection, server-side revocation on logout, idle timeout, and forced re-auth are **enforced** on `/auth/refresh` (D-017 auth package, July 2026); MFA + email verification remain deferred per Master Spec §2 |
