...
| Feature flags     | `SHIELD_AUTH_REQUIRE_MFA`, `SHIELD_AUTH_REQUIRE_EMAIL_VERIFY`, `SHIELD_EMAIL_DELIVERY_ENABLED`            | MFA/email-verify implemented (D-017); `REQUIRE_MFA` only drives a frontend nudge, `REQUIRE_EMAIL_VERIFY` gates login with 403 |
...
| MFA / email       | `JWT_MFA_CHALLENGE_TTL_SECONDS`, `SHIELD_FRONTEND_BASE_URL`                                               | TOTP challenge TTL (5 min); base URL for email-verification links                                                             |
...
2. **Auth hardening (D-017 auth package, July 2026).** Compensating controls **enforced today**: 15-minute JWT access-token lifetime and account lockout after 10 failed attempts in 15 minutes; refresh-token rotation with reuse detection (replaying a rotated token revokes the whole session family), server-side revocation on logout, idle timeout (`SHIELD_IDLE_TIMEOUT_SECONDS`), and forced re-auth (`SHIELD_FORCED_REAUTH_SECONDS`), all checked on `/auth/refresh` against the `refresh_tokens` records. **MFA + email verification now shipped:** enrolled users complete a TOTP (`pyotp`) second factor at login (`/auth/mfa/enroll` → `activate` → login `challenge_token` → `/auth/mfa/verify`); every registration mints a hashed email-verification token and best-effort emails a `/verify-email` link. `SHIELD_AUTH_REQUIRE_EMAIL_VERIFY=true` refuses login (403) for unverified accounts after the password check. `SHIELD_AUTH_REQUIRE_MFA=true` currently only drives a frontend enrollment nudge (`mfa_setup_required`); server-side enforcement of universal MFA is a documented future hardening step (see `DECISIONS.md` D-017).
